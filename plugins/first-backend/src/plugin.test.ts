...
describe('plugin', () => {
    it('should create and read TODO items', async () => {
        const { server } = await startTestBackend({
            features: [firstPlugin],
        });

        await request(server).get('/api/first/todos').expect(200, {
            items: [],
        });

        const createRes = await request(server)
            .post('/api/first/todos')
            .send({ title: 'My Todo' });
        ...
        await request(server)
            .get(`/api/first/todos/${createdTodoItem.id}`)
    ...
    it('should create TODO item with catalog information', async () => {
        ...
        const createRes = await request(server).post('/api/first/todos').send({
            title: 'My Todo',
